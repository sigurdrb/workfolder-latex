...
\begin{tikzpicture}[remember picture]
	...
	% --------------- start raspberrypi ---------------
	% host description
	\node[host description, right=8cm of router-description] (raspberrypi-description) {%
		\begin{tcolorbox}[
			adjusted title= ScaleAQ Master PLC,
			host description box
		]
		IP: \hspace*{\fill} \ip{X.X.X.X}{subnet-home}\\
		\tcblower
	...
	\node[
		...
		\begin{tcboxedraster}[services raster]{services box}
			\tcbox[enhanced, remember as=raspberrypi-pihole]{\service{}{Automated dead fish handler}}
		\end{tcboxedraster}
	};
	...
	\node[anchor=north] at (raspberrypi-services.north east -| raspberrypi-description.east) (raspberrypi-interfaces2) {
		\begin{tcbitemize}[interfaces raster]
			\\[1cm]
			\tcbitem[remember as=raspberrypi-eth3] \faIcon{ethernet}\hfill eth3
		\end{tcbitemize}
	};
...
% --------------- begin liftUp ---------------
	% host description
	\node[host description, right=8cm of acid-host-box] (liftUp-description) {%
		\begin{tcolorbox}[
	...
	% services
	\node[
		below=0em of liftUp-description.south east,
		anchor=north east
	] (liftUp-services) {
		\begin{tcboxedraster}[services raster]{services box}
	...
	% network interfaces
	\node[anchor=north] at (liftUp-services.north west -| liftUp-description.west) (liftUp-interfaces) {
		\begin{tcbitemize}[interfaces raster]
	...
	\node[
		host box,
		fit=(liftUp-description.center) (liftUp-services.south) (liftUp-services.east) (liftUp-interfaces.south)
	] (liftUp-host-box) {};
	\end{pgfonlayer}
	% --------------- end LiftUp ---------------
	...
	% --------------- start ScaleServer ---------------
	% host description
	\node[host description, right=3cm of raspberrypi-description] (ScaleServer-description) {%
		\begin{tcolorbox}[
	...
	% services
	\node[
		below=0em of ScaleServer-description.south east,
		anchor=north east
	] (ScaleServer-services) {
		\begin{tcboxedraster}[services raster]{services box}
			\tcbox[enhanced, remember as=ScaleServer-pihole]{\service{}{SiteControl}}
		\end{tcboxedraster}
	};

	
	% network interfaces
	\node[anchor=north] at (ScaleServer-services.north west -| ScaleServer-description.west) (ScaleServer-interfaces) {
		\begin{tcbitemize}[interfaces raster]
			\tcbitem[remember as=ScaleServer-eth0] \faIcon{ethernet}\hfill eth0
			\tcbitem[remember as=ScaleServer-eth1] \faIcon{ethernet}\hfill eth1
		\end{tcbitemize}
	};
	...
	\node[
		host box,
		fit=(ScaleServer-description.center) (ScaleServer-services.south) (ScaleServer-services.east) (ScaleServer-interfaces.south)
	] (ScaleServer-host-box) {};
	\end{pgfonlayer}
	% --------------- end ScaleServer ---------------
	...
	\begin{scope}[ethernet]
		\draw[subnet-home] (raspberrypi-eth0.west) -- ++(-1,0) |- node[above,xshift = -2cm] {OPC UA} (router-eth0.east);
		\draw[subnet-home] (router-eth1.east) -- ++(1,0) |- (acid-eth0.east);
		\draw[subnet-home] (liftUp-eth0.west) -- ++(-1,0) |- node[above,xshift = -1.5cm, yshift = -3cm] {MODBUS TCP/IP} (raspberrypi-eth1.west);
		\draw[subnet-home] (raspberrypi-eth3.east) -| node[above,yshift=0.5cm] {OPC UA} (ScaleServer-eth0.west);
	\end{scope}
